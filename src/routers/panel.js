...
const Products = require("../functions/product");
app.get("/discord", async (req, res) => {
    res.redirect("https://discord.gg/WXCsr8zTN6");
});

app.use((req, res, next) => req.user ? next() : res.redirect("/"));



app.get("/", async (req, res) => {
    const products = await Products.getProducts("all", req.user.PassID)
...
app.get("/apps", async (req, res) => {
    const products = await Products.getProducts("App", req.user.PassID)
...
app.get("/apps/create", async (req, res) => {
...
app.get("/games", async (req, res) => {
    const products = await Products.getProducts("Game", req.user.PassID)
...
app.get("/workshop", async (req, res) => {
    const products = await Products.getProducts("Workshop", req.user.PassID)
...
app.get("/apis", async (req, res) => {
    const products = await Products.getAPIs("all", req.user.PassID)
...
// SOON
app.get(["/team", "/collections", "/events"], async (req, res) => {
    res.reply("panel", {
        URL: "soon"
    })
});
